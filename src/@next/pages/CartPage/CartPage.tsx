...
const getCheckoutButton = (history: History, user?: UserDetails_me | null) => (
  ...
  >
    <FormattedMessage defaultMessage="Start Checkout" />
  </Button>
);

const needHelp = (history: History, user?: UserDetails_me | null) => (
  <Button
    testingContext="proceedToCheckoutButton"
    onClick={() => history.push(user ? `/checkout/` : `/login/`)}
  >
  ...
  if (loaded && items?.length) {
    return (
      <Cart
        ...
        buttonShopping={getShoppingButton(history)}
        buttonHelp = {needHelp(history)}
