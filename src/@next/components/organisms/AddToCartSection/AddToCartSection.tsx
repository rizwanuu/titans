...
const AddToCartSection: React.FC<IAddToCartSection> = ({
  ...
  return (
    <S.AddToCartSelection className="box3">
    {/* <div className="col-lg-4">
      <div className="box3 mt-2">
        <div> <span className="price">$697.75</span><span className="text-muted">/each</span></div>
        <hr className="line3" />
        <div className="box4"> 1 <span className="pc"> pcs </span></div>
        <button className="box-but"> ADD TO CART </button>
      </div>
    </div> */}
      ...
      {isOutOfStock ? (
        ...
        <S.ProductPricing>
          {getProductPrice(productPricing, variantPricing)}<span style={{fontWeight: "normal", fontSize: "18px"}}>/each</span>
        </S.ProductPricing>
      )}
      {noPurchaseAvailable &&
        renderErrorMessage(
